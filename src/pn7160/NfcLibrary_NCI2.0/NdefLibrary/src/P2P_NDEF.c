...
const unsigned char SNEP_PUT_SUCCESS[] = {SNEP_VER10, SNEP_SUCCESS, 0x00,
                                          0x00,       0x00,         0x00};
const unsigned char SNEP_PUT_CONTINUE[] = {SNEP_VER10, SNEP_CONTINUE, 0x00,
                                           0x00,       0x00,          0x00};
const unsigned char LLCP_CONNECT_SNEP[] = {0x11, 0x20};
const unsigned char LLCP_I_SNEP_PUT_HEADER[] = {SNEP_VER10, SNEP_PUT, 0x00,
                                                0x00,       0x00,     0x00};
...
/* Defines the number of symmetry exchanges is expected before initiating the
 * NDEF push (to allow a remote phone to beam an NDEF message first) */
...
typedef void P2P_NDEF_PullCallback_t(unsigned char*, unsigned short,
                                     unsigned int);
...
static void ParseLlcp(unsigned char* pBuf, unsigned short BufSize,
                      P2P_NDEF_LlcpHeader_t* pLlcpHeader) {
  pLlcpHeader->Dsap = pBuf[0] >> 2;
  pLlcpHeader->Pdu = ((pBuf[0] & 3) << 2) + (pBuf[1] >> 6);
  pLlcpHeader->Ssap = pBuf[1] & 0x3F;

  if ((pLlcpHeader->Pdu == PAX) || (pLlcpHeader->Pdu == CONNECT) ||
      (pLlcpHeader->Pdu == CC) || (pLlcpHeader->Pdu == SNL)) {
    uint8_t i = 2;
    while (i < BufSize) {
      switch (pBuf[i]) {
        case VERSION:
        ...
        case RW:
          ...
          break;
        case SN:
          memcpy(pLlcpHeader->Sn, &pBuf[i + 2],
                 pBuf[i + 1] < sizeof(pLlcpHeader->Sn)
                     ? pBuf[i + 1]
                     : sizeof(pLlcpHeader->Sn));
          break;
        default:
          break;
      }
...
bool P2P_NDEF_SetMessage(unsigned char* pMessage, unsigned int Message_size,
                         void* pCb) {
  if (pMessage != NULL) {
  ...
  switch (LlcpHeader.Pdu) {
    case CONNECT:
      ...
      if ((LlcpHeader.Dsap == SAP_SNEP) ||
          (memcmp(LlcpHeader.Sn, "urn:nfc:sn:snep", 15) == 0)) {
        /* Only accept the connection is application is registered for NDEF
         * reception */
    ...
    case I:
      ...
      if (gP2P_DEF_fragmentation == 0) {
        ...
        if ((pCmd[3] == SNEP_VER10) && (pCmd[4] == SNEP_PUT)) {
          gP2P_NDEF_size =
              pCmd[8] + (pCmd[7] << 8) + (pCmd[6] << 16) + (pCmd[5] << 24);

          /* Notify application of the NDEF reception */
          if (pP2P_NDEF_PullCb != NULL)
            pP2P_NDEF_PullCb(&pCmd[9], Cmd_size - 9, gP2P_NDEF_size);
        ...
        /* Notify application of the NDEF reception */
        if (pP2P_NDEF_PullCb != NULL)
          pP2P_NDEF_PullCb(&pCmd[3], Cmd_size - 3, gP2P_NDEF_size);
    ...
    case CC:
      /* Connection to remote SNEP server completed, send NDEF message inside
       * SNEP PUT request */
      eP2P_SnepClient_State = SnepClientConnected;
      /* reset frame numbering */
      gP2P_NDEF_curNR = 0;
    ...
    switch (eP2P_SnepClient_State) {
      ...
      case NdefMsgSending:
        ...
        if (gP2P_NDEF_ptr + P2P_NDEF_SNEP_MAX_FRAME_SIZE >= NdefMessage_size) {
          memcpy(&pRsp[3], pNdefMessage + gP2P_NDEF_ptr,
                 NdefMessage_size - gP2P_NDEF_ptr);
