...
#include "driver_config.h" /* SET_OUT, High, LOW */
#include "tml.h"
#include "tool.h"
...
static const struct gpio_dt_spec PORT_STATUS_LED =
...
static bool NxpNci_CheckDevPres(void) {
  ...
  if (NbBytes != 0) {
    ...
    else if ((Answer[0] == 0x60) && (Answer[1] == 0x00) &&
             (Answer[5] == 0x20)) {
...
static bool NxpNci_WaitForReception(uint8_t* pRBuff, uint16_t RBuffSize,
                                    uint16_t* pBytesread, uint16_t timeout) {
  tml_Receive(pRBuff, RBuffSize, pBytesread, timeout);
  if (*pBytesread == 0) {
    return NXPNCI_ERROR;
  }

  if (*pBytesread == 0xFFFF) {
    return NXPNCI_ERROR;
  }
...
static bool NxpNci_HostTransceive(uint8_t* pTBuff, uint16_t TbuffLen,
                                  uint8_t* pRBuff, uint16_t RBuffSize,
                                  uint16_t* pBytesread) {
...
static void NxpNci_FillInterfaceInfo(NxpNci_RfIntf_t* pRfIntf, uint8_t* pBuf) {
  ...
  switch (pRfIntf->ModeTech) {
    case (MODE_POLL | TECH_PASSIVE_NFCA):
      ...
      temp++;
      memcpy(pRfIntf->Info.NFC_APP.NfcId, &pBuf[3],
             pRfIntf->Info.NFC_APP.NfcIdLen);
      temp += pBuf[2];
    ...
    case (MODE_POLL | TECH_PASSIVE_NFCB):
      ...
      memcpy(pRfIntf->Info.NFC_BPP.SensRes, &pBuf[1],
             pRfIntf->Info.NFC_BPP.SensResLen);
      temp = pBuf[0] + 4;
      if (pBuf[temp] != 0) {
        temp++;
    ...
    case (MODE_POLL | TECH_PASSIVE_NFCF):
      ...
      memcpy(pRfIntf->Info.NFC_FPP.SensRes, &pBuf[2],
             pRfIntf->Info.NFC_FPP.SensResLen);
      break;
...
void NxpNci_ProcessCardMode(NxpNci_RfIntf_t RfIntf) {
  ...
  while (NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                 TIMEOUT_2S) == NXPNCI_SUCCESS) {
    /* is RF_DEACTIVATE_NTF ? */
    if ((Answer[0] == 0x61) && (Answer[1] == 0x06)) {
      if (FirstCmd) {
        /* Restart the discovery loop */
        NxpNci_HostTransceive(NCIStopDiscovery, sizeof(NCIStopDiscovery),
                              Answer, sizeof(Answer), &AnswerSize);
        do {
          if ((Answer[0] == 0x41) && (Answer[1] == 0x06)) break;
          NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                  TIMEOUT_100MS);
        } while (AnswerSize != 0);
        NxpNci_HostTransceive(NCIStartDiscovery, NCIStartDiscovery_length,
                              Answer, sizeof(Answer), &AnswerSize);
      }
    ...
    /* is DATA_PACKET ? */
      ...
      NxpNci_HostTransceive(Cmd, CmdSize + 3, Answer, sizeof(Answer),
                            &AnswerSize);
    }
    FirstCmd = false;
  }
...
void NxpNci_ProcessP2pMode(NxpNci_RfIntf_t RfIntf) {
  ...
  if ((RfIntf.ModeTech & MODE_LISTEN) != MODE_LISTEN) {
    ...
    NxpNci_HostTransceive(NCILlcpSymm, sizeof(NCILlcpSymm), Answer,
  ...
  while (status == NXPNCI_SUCCESS) {
    ...
    if ((Answer[0] == 0x00) && (Answer[1] == 0x00)) {
      ...
      Cmd[1] = (CmdSize & 0xFF00) >> 8;
      Cmd[2] = CmdSize & 0x00FF;
      status = NxpNci_HostTransceive(Cmd, CmdSize + 3, Answer, sizeof(Answer),
                                     &AnswerSize);
    }
    ...
    /* is RF_DISCOVERY_NTF ? */
    else if ((Answer[0] == 0x61) &&
             ((Answer[1] == 0x05) || (Answer[1] == 0x03))) {
      do {
        if ((Answer[0] == 0x61) &&
            ((Answer[1] == 0x05) || (Answer[1] == 0x03))) {
          if ((Answer[6] & MODE_LISTEN) != MODE_LISTEN)
            restart = true;
          else
            restart = false;
        }
        status = NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                         TIMEOUT_100MS);
      } while (AnswerSize != 0);
      /* Come back to discovery state */
      break;
...
bool NxpNci_ReaderTagCmd(unsigned char* pCommand, unsigned char CommandSize,
...
static void NxpNci_ReadNdef(NxpNci_RfIntf_t RfIntf) {
  uint8_t Answer[MAX_NCI_FRAME_SIZE] = {0, 0};
  uint16_t AnswerSize;
  uint8_t Cmd[MAX_NCI_FRAME_SIZE] = {0, 0};
  uint16_t CmdSize = 0;

  RW_NDEF_Reset(RfIntf.Protocol);

  while (1) {
    RW_NDEF_Read_Next(&Answer[3], Answer[2], &Cmd[3],
                      (unsigned short*)&CmdSize);
    if (CmdSize == 0) {
...
static void NxpNci_WriteNdef(NxpNci_RfIntf_t RfIntf) {
  ...
  while (1) {
    RW_NDEF_Write_Next(&Answer[3], Answer[2], &Cmd[3],
                       (unsigned short*)&CmdSize);
    if (CmdSize == 0) {
...
static void NxpNci_PresenceCheck(NxpNci_RfIntf_t RfIntf) {
  ...
  switch (RfIntf.Protocol) {
    case PROT_T1T:
      do {
        k_msleep(500);
        NxpNci_HostTransceive(NCIPresCheckT1T, sizeof(NCIPresCheckT1T), Answer,
                              sizeof(Answer), &AnswerSize);
        NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                TIMEOUT_100MS);
    ...
    case PROT_T2T:
      do {
        gpio_pin_set_dt(&PORT_STATUS_LED, HIGH);
        k_msleep(125);
        gpio_pin_set_dt(&PORT_STATUS_LED, LOW);
        k_msleep(125);
        gpio_pin_set_dt(&PORT_STATUS_LED, HIGH);
        k_msleep(125);
        gpio_pin_set_dt(&PORT_STATUS_LED, LOW);
        k_msleep(125);
        NxpNci_HostTransceive(NCIPresCheckT2T, sizeof(NCIPresCheckT2T), Answer,
                              sizeof(Answer), &AnswerSize);
        NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                TIMEOUT_100MS);
      } while ((Answer[0] == 0x00) && (Answer[1] == 0x00) &&
    ...
    case PROT_T3T:
      do {
        k_msleep(500);
        NxpNci_HostTransceive(NCIPresCheckT3T, sizeof(NCIPresCheckT3T), Answer,
                              sizeof(Answer), &AnswerSize);
        NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                TIMEOUT_100MS);
      } while ((Answer[0] == 0x61) && (Answer[1] == 0x08) &&
               ((Answer[3] == 0x00) || (Answer[4] > 0x00)));
      break;
    ...
    case PROT_ISODEP:
      do {
        ...
        gpio_pin_set_dt(&PORT_STATUS_LED, LOW);
        k_msleep(125);
        NxpNci_HostTransceive(NCIPresCheckIsoDep, sizeof(NCIPresCheckIsoDep),
                              Answer, sizeof(Answer), &AnswerSize);
        NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                TIMEOUT_100MS);
    ...
    case PROT_T5T:
      do {
        ...
        NxpNci_HostTransceive(NCIPresCheckIso15693,
                              sizeof(NCIPresCheckIso15693), Answer,
                              sizeof(Answer), &AnswerSize);
        status = NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                         TIMEOUT_100MS);
      } while ((status == NXPNCI_SUCCESS) && (Answer[0] == 0x00) &&
               (Answer[1] == 0x00) && (Answer[AnswerSize - 1] == 0x00));
      break;
    ...
    case PROT_MIFARE:
      do {
        ...
        k_msleep(125);
        /* Deactivate target */
        NxpNci_HostTransceive(NCIDeactivate, sizeof(NCIDeactivate), Answer,
                              sizeof(Answer), &AnswerSize);
        NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                TIMEOUT_100MS);
        /* Reactivate target */
        NxpNci_HostTransceive(NCISelectMIFARE, sizeof(NCISelectMIFARE), Answer,
                              sizeof(Answer), &AnswerSize);
        NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                TIMEOUT_100MS);
...
void NxpNci_ProcessReaderMode(NxpNci_RfIntf_t RfIntf,
                              NxpNci_RW_Operation_t Operation) {
#ifndef REMOVE_NDEF_SUPPORT
...
bool NxpNci_ReaderActivateNext(NxpNci_RfIntf_t* pRfIntf) {
  ...
  NxpNci_HostTransceive(NCIStopDiscovery, sizeof(NCIStopDiscovery), Answer,
                        sizeof(Answer), &AnswerSize);
  if ((Answer[0] != 0x41) && (Answer[1] != 0x06) && (Answer[3] != 0x00))
    return NXPNCI_ERROR;
  NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_100MS);
  ...
  NxpNci_HostTransceive(NCIRfDiscoverSelect, sizeof(NCIRfDiscoverSelect),
                        Answer, sizeof(Answer), &AnswerSize);
...
bool NxpNci_ReaderReActivate(NxpNci_RfIntf_t* pRfIntf) {
  ...
  NxpNci_HostTransceive(NCIActivate, sizeof(NCIActivate), Answer,
                        sizeof(Answer), &AnswerSize);
  NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_100MS);
...
bool NxpNci_Connect(void) {
  ...
  NxpNci_HostTransceive(NCICoreInit_2_0, sizeof(NCICoreInit_2_0), Answer,
                        sizeof(Answer), &AnswerSize);
...
bool NxpNci_ConfigureSettings(void) {
  ...
  if (sizeof(NxpNci_CORE_STANDBY) != 0) {
    ...
    NxpNci_HostTransceive(NxpNci_CORE_STANDBY, sizeof(NxpNci_CORE_STANDBY),
                          Answer, sizeof(Answer), &AnswerSize);
  ...
  if (sizeof(NxpNci_CORE_CONF) != 0) {
    NxpNci_HostTransceive(NxpNci_CORE_CONF, sizeof(NxpNci_CORE_CONF), Answer,
                          sizeof(Answer), &AnswerSize);
  ...
  NxpNci_HostTransceive(NCIReadTS, sizeof(NCIReadTS), Answer, sizeof(Answer),
                        &AnswerSize);
  if ((Answer[0] != 0x40) || (Answer[1] != 0x03) || (Answer[3] != 0x00))
    return NXPNCI_ERROR;

  /* Then compare with current build timestamp, and check RF settings restore
   * flag */
  if (!memcmp(&Answer[8], NxpNci_SettingCurrentTS,
              sizeof(NxpNci_SettingCurrentTS)) &&
      (gRfSettingsRestored_flag == false)) {
    ...
    if (sizeof(NxpNci_CORE_CONF_EXTN) != 0) {
      isResetRequired = true;
      NxpNci_HostTransceive(NxpNci_CORE_CONF_EXTN,
                            sizeof(NxpNci_CORE_CONF_EXTN), Answer,
                            sizeof(Answer), &AnswerSize);
      if ((Answer[0] != 0x40) || (Answer[1] != 0x02) || (Answer[3] != 0x00) ||
          (Answer[4] != 0x00))
        return NXPNCI_ERROR;
    ...
    if (sizeof(NxpNci_TVDD_CONF) != 0) {
      isResetRequired = true;
      NxpNci_HostTransceive(NxpNci_TVDD_CONF, sizeof(NxpNci_TVDD_CONF), Answer,
                            sizeof(Answer), &AnswerSize);
      if ((Answer[0] != 0x40) || (Answer[1] != 0x02) || (Answer[3] != 0x00) ||
          (Answer[4] != 0x00))
        return NXPNCI_ERROR;
    ...
    if (sizeof(NxpNci_RF_CONF) != 0) {
      isResetRequired = true;
      NxpNci_HostTransceive(NxpNci_RF_CONF, sizeof(NxpNci_RF_CONF), Answer,
                            sizeof(Answer), &AnswerSize);
      if ((Answer[0] != 0x40) || (Answer[1] != 0x02) || (Answer[3] != 0x00) ||
          (Answer[4] != 0x00))
        return NXPNCI_ERROR;
    ...
    memcpy(&NCIWriteTS[7], NxpNci_SettingCurrentTS,
           sizeof(NxpNci_SettingCurrentTS));
    NxpNci_HostTransceive(NCIWriteTS, sizeof(NCIWriteTS), Answer,
                          sizeof(Answer), &AnswerSize);
    if ((Answer[0] != 0x40) || (Answer[1] != 0x02) || (Answer[3] != 0x00) ||
        (Answer[4] != 0x00))
      return NXPNCI_ERROR;
  ...
  if (isResetRequired) {
    ...
    NxpNci_HostTransceive(NCICoreReset, sizeof(NCICoreReset), Answer,
                          sizeof(Answer), &AnswerSize);
...
bool NxpNci_ConfigureMode(unsigned char mode) {
  ...
  uint8_t NCISetConfig_NFCA_SELRSP[] = {0x20, 0x02, 0x04, 0x01,
                                        0x32, 0x01, 0x00};
#endif  // #if !((defined REMOVE_P2P_SUPPORT) && (defined
        // REMOVED_CARDEMU_SUPPORT))
  ...
  if (mode == NXPNCI_MODE_RW) {
    NxpNci_HostTransceive(NCIPropAct, sizeof(NCIPropAct), Answer,
                          sizeof(Answer), &AnswerSize);
...
#endif  // #ifndef REMOVE_RW_SUPPORT
#if !((defined REMOVE_P2P_SUPPORT) && (defined REMOVED_CARDEMU_SUPPORT) && \
      (defined REMOVE_RW_SUPPORT))
  if (Item != 0) {
    memcpy(Command, NCIDiscoverMap, sizeof(NCIDiscoverMap));
    Command[2] = 1 + (Item * 3);
    Command[3] = Item;
    NxpNci_HostTransceive(Command, 3 + Command[2], Answer, sizeof(Answer),
                          &AnswerSize);
    if ((Answer[0] != 0x41) || (Answer[1] != 0x00) || (Answer[3] != 0x00))
      return NXPNCI_ERROR;
  }
#endif  // #if !((defined REMOVE_P2P_SUPPORT) && (defined
        // REMOVED_CARDEMU_SUPPORT) && (defined REMOVE_RW_SUPPORT))
  ...
  if (Item != 0) {
    ...
    NxpNci_HostTransceive(Command, 3 + Command[2], Answer, sizeof(Answer),
                          &AnswerSize);
    if ((Answer[0] != 0x41) || (Answer[1] != 0x01) || (Answer[3] != 0x00))
      return NXPNCI_ERROR;
  }
#endif  // #if !((defined REMOVE_P2P_SUPPORT) && (defined
        // REMOVED_CARDEMU_SUPPORT))
  ...
  if (NCISetConfig_NFCA_SELRSP[6] != 0x00) {
    NxpNci_HostTransceive(NCISetConfig_NFCA_SELRSP,
                          sizeof(NCISetConfig_NFCA_SELRSP), Answer,
                          sizeof(Answer), &AnswerSize);
    if ((Answer[0] != 0x40) || (Answer[1] != 0x02) || (Answer[3] != 0x00))
      return NXPNCI_ERROR;
  }
  ...
  if (mode & NXPNCI_MODE_P2P) {
    NxpNci_HostTransceive(NCISetConfig_NFC, sizeof(NCISetConfig_NFC), Answer,
                          sizeof(Answer), &AnswerSize);
    if ((Answer[0] != 0x40) || (Answer[1] != 0x02) || (Answer[3] != 0x00))
      return NXPNCI_ERROR;
  }
...
bool NxpNci_StartDiscovery(const unsigned char* pTechTab,
  ...
  NxpNci_HostTransceive(NCIStartDiscovery, NCIStartDiscovery_length, Answer,
                        sizeof(Answer), &AnswerSize);
...
#endif  // #ifndef REMOVE_P2P_SUPPORT
  do {
    if (NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                TIMEOUT_INFINITE) == NXPNCI_ERROR)
      return NXPNCI_ERROR;
          ...
          if (AnswerSize != 0) {
            ...
            while (Answer != 0)
              NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                      TIMEOUT_100MS);
            /* Restart the discovery loop */
            NxpNci_HostTransceive(NCIRestartDiscovery,
                                  sizeof(NCIRestartDiscovery), Answer,
                                  sizeof(Answer), &AnswerSize);
            NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize,
                                    TIMEOUT_100MS);
          }
...
bool NxpNci_FactoryTest_Prbs(NxpNci_TechType_t type, NxpNci_Bitrate_t bitrate) {
  ...
  NxpNci_HostTransceive(NCIPrbs, sizeof(NCIPrbs), Answer, sizeof(Answer),
                        &AnswerSize);
  if ((Answer[0] != 0x4F) || (Answer[1] != 0x30) || (Answer[3] != 0x00))
    return NXPNCI_ERROR;
...
bool NxpNci_FactoryTest_RfOn(void) {
  ...
  NxpNci_HostTransceive(NCIRfOn, sizeof(NCIRfOn), Answer, sizeof(Answer),
                        &AnswerSize);
