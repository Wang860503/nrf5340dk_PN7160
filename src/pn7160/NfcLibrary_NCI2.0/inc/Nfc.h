...
/*
 * Process the operation identified as parameter with discovered remote NFC tag
 * (function is blocking until the end of the operation)
 * - RfIntf: discovered NFC device properties
 * - Operation: select operation to be done with the remote NFC tag
 *  o READ_NDEF: extract NDEF message from the tag, previously registered
 * callback function will be called whenever complete NDEF message is found. o
 * WRITE_NDEF: write previously registered NDEF message to the tag o
 * PRESENCE_CHECK: perform presence check until tag has been removed (function
 * is blocking until card is removed)
 */
...
/*
 * Perform activation of the next tag (in case of multiple tag detection or
 * multi-protocol tag)
 * - pRfIntf: filled with discovered NFC remote device properties
 * return NFC_SUCCESS or NFC_ERROR
 */
...
bool T4T_NDEF_EMU_SetMessage(unsigned char* pMessage,
                             unsigned short Message_size, void* pCb);
#endif  // #ifndef REMOVE_NDEF_SUPPORT

/*
 * Expose the previously registered NDEF message to discovered remote NFC reader
 * (function is blocking until the remote reader is lost):
 * - RfIntf: discovered NFC device properties
 */
...
/*
 * Register function called when NDEF message is received from remote NFC Peer
 * to peer Device
 * - pCb: pointer to function to be called back when NDEF message has been
 * received
 */
